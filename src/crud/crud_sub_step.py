...
class CrudSubStep(CRUDBase[SubStep, SubStepCreate, SubStepUpdate]):
    def create_sub_steps(
            self, db: Session, *, new_data: Optional[SubStepCreate]):
        # проверка уникальности трех полей
        if db.query(SubStep).filter(
                SubStep.name == new_data.name).first() is not None:
            return None, -1261, None
        db_obj = super().create(db=db, obj_in=new_data)
        return db_obj, 0, None

    def update_sub_steps(
            self, db: Session, *, new_data: Optional[SubStepUpdate],
            sub_step_id: int):
        # проверить есть ли model с таким id
        this_obj = (db.query(SubStep).filter(
            SubStep.id == sub_step_id).first())
        if this_obj is None:
