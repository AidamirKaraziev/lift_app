...
class CrudReasonFault(CRUDBase[ReasonFault, ReasonFaultCreate, ReasonFaultUpdate]):
    ...
    def update(self, db: Session, *, new_data: Optional[ReasonFaultUpdate], obj_id: int):
        ...
        if r_c.name != new_data.name:
            if db.query(ReasonFault).filter(ReasonFault.name == new_data.name, ReasonFault.id != obj_id).first() is not None:
