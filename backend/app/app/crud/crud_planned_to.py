...
from app.models import UniversalUser, PlannedTO, Object
...
class CrudPlannedTO(CRUDBase[PlannedTO, PlannedTOCreate, PlannedTOUpdate]):
    ...
    def create_planned_to(self, db: Session, *, new_data: PlannedTOCreate):
        ...
        if new_data.year is not None and new_data.object_id is not None:
            constrain = db.query(PlannedTO).filter(PlannedTO.year == new_data.year,
    ...
    def update_planned_to(self, db: Session, *, new_data: Optional[PlannedTOUpdate], planned_to_id: int):
        # проверка наличия планового ТО
        this_planned_to, code, indexes = self.getting_planned_to(db=db, planned_to_id=planned_to_id)
        if code != 0:
        ...
        list_planned_to = [new_data.january_to_id, new_data.february_to_id, new_data.march_to_id, new_data.april_to_id,
                           new_data.may_to_id, new_data.june_to_id, new_data.july_to_id, new_data.august_to_id,
                           new_data.september_to_id, new_data.october_to_id, new_data.november_to_id,
                           new_data.december_to_id]
        for to in list_planned_to:
            if to is not None:
                obj, code, indexes = crud_acts_fact.getting_act_fact(db=db, act_fact_id=to)
                if code != 0:
